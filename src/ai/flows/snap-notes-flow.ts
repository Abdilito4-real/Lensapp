...
const SnapNotesInputSchema = z.object({
  photoDataUri: z
    .string()
    .describe(
      "A photo of notes, a textbook, or a slide, as a data URI that must include a MIME type and use Base64 encoding. Expected format: 'data:<mimetype>;base64,<encoded_data>'."
    ),
  action: z.enum(['summarize', 'flashcards', 'quiz']).describe("The desired study tool to generate."),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert academic assistant. Your task is to analyze the provided image of study material and generate a specific type of study tool based on the user's request.

Analyze the text content from the image.
...
- If 'action' is 'flashcards', identify key terms and definitions or questions and answers, and format them as an array of flashcards in the 'flashcards' field. Aim for 5-10 key flashcards.
...
Image with study material: {{media url=photoDataUri}}`,
});
